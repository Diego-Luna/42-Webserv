...
void Parsing::f_check_proms(std::string line){
  ...
  if (f_counter_clean_worlds(line, "server") == 1 && f_check_clean_line(line, "server") == true
    ...
    return ;
  }else if (f_counter_clean_worlds(line, "index") == 1 &&
    f_run_each_words(line, &num_worlds, line.find("index") + 5, NULL) == true &&
     num_worlds == 1 && f_check_clean_line(line, "index") == true){
    return ;
